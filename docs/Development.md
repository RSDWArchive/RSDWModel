# RSDWModel Development Notes

This is the maintainer-facing pipeline reference formerly kept in the root
README. It intentionally includes local setup paths, cache locations, update
commands, publishing details, and operational notes for this working copy.
...
The `/Avatar/` page is a static Three.js assembly viewer. It loads several
Draco-compressed glTF assets into one scene so player body, head, hair, beard,
torso, legs, helmet, cape, and held weapon pieces can be layered together.

Avatar data is generated by `tools/generate_avatar_index.py`. The generator
reads `website/model-index.json`, selects Player assets plus M/F/U medium armor
assets and held equipment metadata from the current `WebAssets` / RSDWArchive
...
weapon mesh when available, falls back to the item's static mesh otherwise, and
records right-hand, left-hand, and two-handed slot rules for the Avatar UI.
Each held item is exposed in both hand dropdowns so users can experiment with
off-hand and mirrored setups. The runtime keeps natural-hand URLs stable and
adds collapsed per-hand adjustment controls for position and rotation tweaks;
adjustments are serialized in the Avatar hash as `<hand>Offset` and
`<hand>Rotation` triples.

`tools/UpdateGameData.py` regenerates both website indexes after the web asset
stage unless `--skip-website-index` is passed. If a local RSDWArchive checkout
has exported curve JSON, pass `--archive-json-root <path-to-version-json>` to
use it explicitly for Avatar skin, hair, and eye palette generation. Without
