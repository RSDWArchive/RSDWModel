...
torso, legs, helmet, and cape pieces can be layered together.
...
assets from the current `WebAssets/` corpus, writes `website/avatar-index.json`,
and creates avatar-only color variants under
`<version>/WebAssets/avatar/textures/`. Runtime color swaps use those generated
textures; the original model glTF files are not mutated.
